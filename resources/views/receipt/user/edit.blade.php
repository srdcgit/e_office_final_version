...
@section('file_title', 'Edit Electronic Receipt')
        ...
        <div class="section-body col-lg-6" style="padding-right:0">
            <div class="col-md-12 m-auto">
                <div class="card">
                    ...
                    {{ Form::model($receipt, ['route' => ['receipt.update', $receipt->id], 'method' => 'PUT', 'files' => true, 'enctype' => 'multipart/form-data']) }}
                    <div class="card-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-4 m-b-10">
                                    {{ Form::label('receipt_file', __('UPLOAD'), ['class' => 'form-label']) }}
                                    {{ Form::file('receipt_file', [
                                        'class' => 'form-control',
                                        'id' => 'fileUpload',
                                        'accept' => '.pdf',
                        ...
                        <iframe id="pdf-viewer" style="height: 153vh; width: 102%; border: none;"
                            src="{{ asset('public/assets/receipt/upload/' . $receipt->receipt_file) }}">
                        </iframe>
                                    ...
                                    <select name="vip" id="vip" class="form-control">
                                        ...
                                        @foreach ($vip as $vips)
                                            <option value="{{ $vips->id }}"
                                                @if ($receipt->vip == $vips->id) selected @endif>{{ $vips->name }}
                                        @endforeach
                                    </select>
                                    @error('vip')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="row">
                                        ...
                                        <select name="category" id="category" class="form-control">
                                            ...
                                            @foreach ($category as $categorys)
                                                <option value="{{ $categorys->id }}"
                                                    @if ($receipt->category == $categorys->id) selected @endif>
                                                    {{ $categorys->name }}
                                            @endforeach
                                        </select>
                                        @error('department')
                                    ...
                                    <div class="card-footer">
                                        <div class="float-end">
                                            ...
                                            <button type="submit"
                                                class="btn btn-primary mb-3">{{ __('Update') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
